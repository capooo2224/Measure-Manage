...
class FaceTrackerEngine:
    ...
    def get_all_expected_students(self):
        """Returns unique dictionary entries for all pre-registered students."""
        unique_students = {}
        for item in self.profile_metadata:
            if item["id"] not in unique_students:
                unique_students[item["id"]] = item["name"]
        return [
            {"id": sid, "name": name} for sid, name in unique_students.items()
        ]
